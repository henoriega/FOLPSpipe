...
PREFERRED_BACKEND = os.environ.get("FOLPS_BACKEND", "numpy")
...
class RSDMultipolesPowerSpectrumCalculator:
    ...
    Attributes:
        ...
        bias_scheme (str): The bias scheme to use (default: 'folps').
        nmu (int): Number of cosine angles (default is 6).
        ells (tuple): Multipole orders to compute (default is (0, 2, 4)).
        IR_resummation (bool): Whether to apply IR resummation (default is True).
    """
    
    def __init__(self, kobs, qpar, qper, pars, table, table_now):
        self.kobs = kobs
        self.qpar = qpar
        self.qper = qper
        #self.pars = pars
        #self.bias_scheme = bias_scheme
        self.table = table
        self.table_now = table_now
        #self.nmu = nmu
        #self.ells = ells
        #self.IR_resummation = IR_resummation
        
        self.pars = self.set_bias_scheme(pars)
    ...
    def get_rsd_pkell(self, kobs, qpar, qper, pars, table, table_now, nmu=6, ells=(0, 2, 4), IR_resummation=True):
