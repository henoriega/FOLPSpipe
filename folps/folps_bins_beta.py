...
# -----------------------------------------------------------------------------
# Binned FoG / damping helpers
# -----------------------------------------------------------------------------
# ``bins_exp`` interprets ``d_damp`` as an X(k) table and applies the same
# exponential damping as the old ``exp`` mode, but with X evaluated at each k.
# ``bins`` interprets ``d_damp`` as a tabulated direct damping amplitude W(k)
# and follows the direct prescription W(k,mu) = (mu * W(k))**2 requested by the
# user.  Several aliases are accepted for convenience/backwards experimentation.
_DAMPING_BINS_EXP_ALIASES = {
    'bins_exp', 'bin_exp', 'binned_exp', 'wbins_exp', 'w_bins_exp', 'w_bin_exp',
    'Wbins_exp', 'W_bins_exp', 'W_bin_exp'
}
_DAMPING_BINS_ALIASES = {
    'bins', 'bin', 'binned', 'wbins', 'w_bins', 'w_bin',
    'Wbins', 'W_bins', 'W_bin'
}


def _damping_key(damping):
    """Return a normalized damping key, preserving None."""
    if damping is None:
        return None
    return str(damping)


def _is_binned_damping_exp(damping):
    return _damping_key(damping) in _DAMPING_BINS_EXP_ALIASES


def _is_binned_damping_direct(damping):
    return _damping_key(damping) in _DAMPING_BINS_ALIASES


def _is_binned_damping(damping):
    return _is_binned_damping_exp(damping) or _is_binned_damping_direct(damping)


def _validate_binned_damping(k_damp, d_damp):
    """Validate and sort user supplied binned damping arrays."""
    if k_damp is None or d_damp is None:
        raise ValueError(
            "For damping='bins'/'W_bin' or damping='bins_exp'/'Wbins_exp', "
            "you must provide k_damp and d_damp."
        )

    k_damp = np.asarray(k_damp)
    d_damp = np.asarray(d_damp)

    if k_damp.ndim != 1 or d_damp.ndim != 1:
        raise ValueError("k_damp and d_damp must be one-dimensional arrays.")
    if len(k_damp) != len(d_damp):
        raise ValueError("k_damp and d_damp must have the same length.")
    if len(k_damp) < 2:
        raise ValueError("k_damp and d_damp must contain at least two points.")

    order = np.argsort(k_damp)
    k_damp = k_damp[order]
    d_damp = d_damp[order]

    # Duplicate k values make interpolation ambiguous.
    if bool(np.any(np.diff(k_damp) <= 0)):
        raise ValueError("k_damp values must be strictly increasing after sorting.")

    return k_damp, d_damp


def _interp_binned_damping(k, k_damp, d_damp, interpolation_type='linear'):
    """
    Interpolate the tabulated damping values at k.

    The default uses constant edge extrapolation instead of NaNs.  This is safer
    for likelihood calls, where k can be slightly outside the tabulated range.
    """
    k_damp, d_damp = _validate_binned_damping(k_damp, d_damp)
    interpolation_type = 'linear' if interpolation_type is None else interpolation_type

    if interpolation_type == 'linear':
        return np.interp(k, k_damp, d_damp, left=d_damp[0], right=d_damp[-1])

    # Higher-order interpolation is intended for the NumPy backend.  Keep the
    # SciPy route explicit so the default linear path remains backend-friendly.
    try:
        import numpy as _np_native
        interp_func = interp1d(
            _np_native.asarray(k_damp),
            _np_native.asarray(d_damp),
            kind=interpolation_type,
            bounds_error=False,
            fill_value=(float(d_damp[0]), float(d_damp[-1])),
        )
        return np.asarray(interp_func(_np_native.asarray(k)))
    except Exception as exc:
        raise ValueError(
            f"Could not interpolate binned damping with interpolation_type={interpolation_type!r}. "
            "Use interpolation_type='linear' for the most robust backend-compatible option."
        ) from exc


def W_bins_exp(k, mu, k_damp, X_damp, f0=0.5, sigma2w=36, interpolation_type='linear'):
    """
    Exponential binned damping for the power spectrum.

    ``X_damp`` is interpreted as a tabulated X(k).  If X_damp is constant, this
    reproduces the old ``exp`` FoG damping.

    Important: do not squeeze the output.  In the bispectrum calls k carries
    broadcasting axes such as (Nk, 1, Nmu, 1); squeezing would destroy these axes
    and break later multiplications.
    """
    k = np.asarray(k)
    X_i = _interp_binned_damping(k, k_damp, X_damp, interpolation_type)
    l2 = (f0 * k * mu * X_i)**2
    return np.exp(-l2 * sigma2w)


def W_bins(k, mu, k_damp, W_damp, interpolation_type='linear'):
    """
    Direct binned damping for the power spectrum.

    ``W_damp`` is interpreted as the damping at mu=1, W_parallel(k).  The angular
    dependence is chosen so W(k,0)=1 and W(k,1)=W_parallel(k):

        W(k, mu) = W_parallel(k)**mu**2.
    """
    k = np.asarray(k)
    W_i = _interp_binned_damping(k, k_damp, W_damp, interpolation_type)
    W_i = np.clip(W_i, 1.0e-300, None)
    return np.exp(mu**2 * np.log(W_i))


def _W_bins_exp_bispectrum(k1, k2, k3, mu1, mu2, mu3, f, sigma2v,
                           k_damp, X_damp, interpolation_type='linear'):
    """Three-leg version of ``W_bins_exp`` for the bispectrum."""
    X1 = _interp_binned_damping(k1, k_damp, X_damp, interpolation_type)
    X2 = _interp_binned_damping(k2, k_damp, X_damp, interpolation_type)
    X3 = _interp_binned_damping(k3, k_damp, X_damp, interpolation_type)
    l2 = 0.5 * f**2 * (
        (k1 * mu1 * X1)**2 +
        (k2 * mu2 * X2)**2 +
        (k3 * mu3 * X3)**2
    )
    return np.exp(-l2 * sigma2v)


def _W_bins_bispectrum(k1, k2, k3, mu1, mu2, mu3,
                       k_damp, W_damp, interpolation_type='linear'):
    """
    Three-leg direct binned damping for the bispectrum.

    Here W_damp is interpreted as the P(k) damping at mu=1.  The factor 1/2 is
    the same one used in the FOLPS bispectrum exponential damping:

        exp[-0.5 * f^2 sigma_v^2 sum_i (k_i mu_i X_i)^2].
    """
    W1 = _interp_binned_damping(k1, k_damp, W_damp, interpolation_type)
    W2 = _interp_binned_damping(k2, k_damp, W_damp, interpolation_type)
    W3 = _interp_binned_damping(k3, k_damp, W_damp, interpolation_type)
    W1 = np.clip(W1, 1.0e-300, None)
    W2 = np.clip(W2, 1.0e-300, None)
    W3 = np.clip(W3, 1.0e-300, None)
    logW = 0.5 * (mu1**2 * np.log(W1) + mu2**2 * np.log(W2) + mu3**2 * np.log(W3))
    return np.exp(logW)

def W_lor_pk(k, mu, X_FoG_p, f0, sigma2w):
    l2 = (f0 * k * mu * X_FoG_p)**2
    return 1.0 / (1.0 + l2 * sigma2w)

    
def W_lor_bk(k1, k2, k3, mu1, mu2, mu3, X_FoG_b, f0, sigma2v):
    S = (k1 * mu1)**2 + (k2 * mu2)**2 + (k3 * mu3)**2
    l2 = 0.5 * S * (f0 * X_FoG_b)**2
    return 1.0 / (1.0 + l2 * sigma2v)

def W_exp_pk(k, mu, X_FoG_p, f0, sigma2w):
    l2 = (f0 * k * mu * X_FoG_p)**2
    return np.exp(-l2 * sigma2w)


def W_exp_bk(k1, k2, k3, mu1, mu2, mu3, X_FoG_b, f0, sigma2v):
    S = (k1 * mu1)**2 + (k2 * mu2)**2 + (k3 * mu3)**2
    l2 = 0.5 * S * (f0 * X_FoG_b)**2
    return np.exp(-l2 * sigma2v)

def W_infty_pk(k, mu, X_FoG_p, f0, sigma2w):
    c2 = (f0 * k * mu)**2
    denom = 1.0 + c2 * X_FoG_p**2
    return np.exp(-c2 * sigma2w / denom) / np.sqrt(denom)

def W_infty_bk(k1, k2, k3, mu1, mu2, mu3, X_FoG_b, f0, sigma2v):
    S = (k1 * mu1)**2 + (k2 * mu2)**2 + (k3 * mu3)**2
    lam = -0.5 * S * f0**2
    denom = 1.0 - lam * X_FoG_b**2
    return np.exp(lam * sigma2v / denom) / denom**1.5
...
class RSDMultipolesPowerSpectrumCalculator:
    ...
    def get_eft_pkmu(self, kev, mu, pars, table, damping='lor', k_damp=None, d_damp=None, damping_interpolation_method='linear'):
        ...
        def Wlorentz(mu, X_FoG_p):
            return W_lor_pk(kev, mu, X_FoG_p, f0, sigma2w)
            
        def Wexp(mu, X_FoG_p):
            return W_exp_pk(kev, mu, X_FoG_p, f0, sigma2w)
            
        def Winfty(mu, X_FoG_p):
            return W_infty_pk(kev, mu, X_FoG_p, f0, sigma2w)
    

        def Wbins_exp(mu, k_damp, d_damp, interpolation_method='linear'):
            return W_bins_exp(
                kev, mu, k_damp, d_damp,
                f0=f0, sigma2w=sigma2w,
                interpolation_type=interpolation_method,
            )

        def Wbins(mu, k_damp, d_damp, interpolation_method='linear'):
            return W_bins(kev, mu, k_damp, d_damp, interpolation_type=interpolation_method)
        ...
        if self.model == "EFT":
            ...
            if damping is None:
                ...
                W = Winfty(mu, X_FoG_p)
            elif _is_binned_damping_exp(damping):
                W = Wbins_exp(mu, k_damp, d_damp, damping_interpolation_method)
            elif _is_binned_damping_direct(damping):
                W = Wbins(mu, k_damp, d_damp, damping_interpolation_method)
            else:
                W = 1
        elif self.model == "FOLPSD":
            if damping is None:
                print("[FOLPS] For FOLPSD you must specify a damping ('exp', 'lor', 'vdg', 'bins_exp', 'bins'). Default: 'lor'.")
                damping = 'lor'
            if damping == 'exp':
                W = Wexp(mu, X_FoG_p)
            elif damping == 'lor':
    ...
    def get_rsd_pkmu(self, k, mu, pars, table, table_now, IR_resummation=True, damping='lor', k_damp=None, d_damp=None, damping_interpolation_method='linear'):
        ...
        pkmu = ((b1 + fk * mu**2)**2 * (pkl_now + np.exp(-k**2 * sigma2t)*(pkl - pkl_now)*(1 + k**2 * sigma2t))
                 + np.exp(-k**2 * sigma2t) * self.get_eft_pkmu(
                     k, mu, pars, table, damping,
                     k_damp=k_damp, d_damp=d_damp,
                     damping_interpolation_method=damping_interpolation_method)
                 + (1 - np.exp(-k**2 * sigma2t)) * self.get_eft_pkmu(
                     k, mu, pars, table_now, damping,
                     k_damp=k_damp, d_damp=d_damp,
                     damping_interpolation_method=damping_interpolation_method))
        return pkmu

    def get_rsd_pkell(self, kobs, qpar, qper, pars, table, table_now,
                      bias_scheme="folps", damping='lor', k_damp=None, d_damp=None,
                      damping_interpolation_method='linear', nmu=6, ells=(0, 2, 4), IR_resummation=True):
        """
...
def get_rsd_pkell_marg_const(
    ...
    def get_rsd_pkmu_const(k, mu, pars, table, table_now, IR_resummation, damping,
                            k_damp=None, d_damp=None, damping_interpolation_method='linear'):
        # Apply bias scheme
        ...
        pkmu = (
            ...
            + np.exp(-k**2 * sigma2t) * multipoles.get_eft_pkmu(
                k, mu, pars_const, table_interp, damping,
                k_damp=k_damp, d_damp=d_damp,
                damping_interpolation_method=damping_interpolation_method)
            + (1 - np.exp(-k**2 * sigma2t)) * multipoles.get_eft_pkmu(
                k, mu, pars_const, table_now_interp, damping,
                k_damp=k_damp, d_damp=d_damp,
                damping_interpolation_method=damping_interpolation_method)
        )
        return pkmu
...
class BispectrumCalculator:
    ...
    def bispectrum(self, k1, k2, x12, mu1, phi, f, sigma2v, Sigma2, deltaSigma2,
        ...
        Wlor, Wexp, Wvdg, Wbinsexp, Wbinsdirect = 1, 1, 1, 1, 1
        
        l2 = (k1AP * mu1AP)**2 + (k2AP * mu2AP)**2 + (k3AP * mu3AP)**2

        if damping == 'lor':
            Wlor = W_lor_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f0, sigma2v)
            W1   = W_lor_pk(k1AP, mu1AP, X_FoG_b, f0, sigma2v)
            W2   = W_lor_pk(k2AP, mu2AP, X_FoG_b, f0, sigma2v)
            W3   = W_lor_pk(k3AP, mu3AP, X_FoG_b, f0, sigma2v)
        elif damping == 'exp':
            Wexp = W_exp_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f0, sigma2v)
            W1   = W_exp_pk(k1AP, mu1AP, X_FoG_b, f0, sigma2v)
            W2   = W_exp_pk(k2AP, mu2AP, X_FoG_b, f0, sigma2v)
            W3   = W_exp_pk(k3AP, mu3AP, X_FoG_b, f0, sigma2v)
        elif damping == 'vdg':
            Winfty = W_infty_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f0, sigma2v)
            W1     = W_infty_pk(k1AP, mu1AP, X_FoG_b, f0, sigma2v)
            W2     = W_infty_pk(k2AP, mu2AP, X_FoG_b, f0, sigma2v)
            W3     = W_infty_pk(k3AP, mu3AP, X_FoG_b, f0, sigma2v)
        elif _is_binned_damping_exp(damping):
            Wbinsexp = _W_bins_exp_bispectrum(
                k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP,
                f0, sigma2v, k_damp, d_damp,
                interpolation_type=damping_interpolation_method,
            )
            W1 = W_bins_exp(
                k1AP, mu1AP, k_damp, d_damp,
                f0=f0, sigma2w=sigma2v,
                interpolation_type=damping_interpolation_method,
            )
            W2 = W_bins_exp(
                k2AP, mu2AP, k_damp, d_damp,
                f0=f0, sigma2w=sigma2v,
                interpolation_type=damping_interpolation_method,
            )
            W3 = W_bins_exp(
                k3AP, mu3AP, k_damp, d_damp,
                f0=f0, sigma2w=sigma2v,
                interpolation_type=damping_interpolation_method,
            )
        elif _is_binned_damping_direct(damping):
            Wbinsdirect = _W_bins_bispectrum(
                k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP,
                k_damp, d_damp,
                interpolation_type=damping_interpolation_method,
            )
...
#             return W 
        ...
        shot = (
                  W1 * (b1*Bshot + 2.0*Pshot*f1*mu1AP**2) * Z1eft1 * pkIR1
                + W2 * (b1*Bshot + 2.0*Pshot*f2*mu2AP**2) * Z1eft2 * pkIR2
                + W3 * (b1*Bshot + 2.0*Pshot*f3*mu3AP**2) * Z1eft3 * pkIR3
                + Pshot**2
        )
    ...
    def Sugiyama_Bl1l2L(self, k1, k2, f, sigma2v, Sigma2, deltaSigma2, bpars, qpar, qper, 
                        tablesGL, k_pkl_pklnw, damping='lor', k_damp=None, d_damp=None,
                        damping_interpolation_method='linear', renormalize=True, 
        ...
        damping : str
            Damping type: 'lor', 'exp', 'vdg', 'bins_exp', or 'bins'
        renormalize : bool
    ...
    def Scoccimarro_B024(
        ...
        k_damp=None,
        d_damp=None,
        damping_interpolation_method='linear',
...
class BispectrumCalculator_fk:
    ...
    def bispectrum(self, k1, k2, x12, mu1, phi, f, sigma2v, Sigma2, deltaSigma2,
                   bpars, qpar, qperp, k_pkl_pklnw_fk, damping='lor',
                   k_damp=None, d_damp=None, damping_interpolation_method='linear',
                   interpolation_method='cubic'):
        ...
        if damping == 'lor':
            Wlor = W_lor_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f, sigma2v)
            W1   = W_lor_pk(k1AP, mu1AP, X_FoG_b, f, sigma2v)
            W2   = W_lor_pk(k2AP, mu2AP, X_FoG_b, f, sigma2v)
            W3   = W_lor_pk(k3AP, mu3AP, X_FoG_b, f, sigma2v)
        elif damping == 'exp':
            Wexp = W_exp_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f, sigma2v)
            W1   = W_exp_pk(k1AP, mu1AP, X_FoG_b, f, sigma2v)
            W2   = W_exp_pk(k2AP, mu2AP, X_FoG_b, f, sigma2v)
            W3   = W_exp_pk(k3AP, mu3AP, X_FoG_b, f, sigma2v)
        elif damping == 'vdg':
            Winfty = W_infty_bk(k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP, X_FoG_b, f, sigma2v)
            W1     = W_infty_pk(k1AP, mu1AP, X_FoG_b, f, sigma2v)
            W2     = W_infty_pk(k2AP, mu2AP, X_FoG_b, f, sigma2v)
            W3     = W_infty_pk(k3AP, mu3AP, X_FoG_b, f, sigma2v)            
        elif _is_binned_damping_exp(damping):
            Wbinsexp = _W_bins_exp_bispectrum(
                k1AP, k2AP, k3AP, mu1AP, mu2AP, mu3AP,
                f, sigma2v, k_damp, d_damp,
                interpolation_type=damping_interpolation_method,
            )
        elif _is_binned_damping_direct(damping):
            Wbinsdirect = _W_bins_bispectrum(
    ...
    def Sugiyama_Bl1l2L(self, k1, k2, f, sigma2v, Sigma2, deltaSigma2, bpars, qpar, qper, 
                        tablesGL, k_pkl_pklnw_fk, damping='lor', k_damp=None, d_damp=None,
                        damping_interpolation_method='linear', renormalize=True, 
                        multipoles=['B000', 'B202'], interpolation_method='linear',precision=None):
        """
        Compute requested Sugiyama bispectrum multipoles.
